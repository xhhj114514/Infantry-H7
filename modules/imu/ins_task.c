...
void INS_Task(void)
{
    ...
    if ((count % 1) == 0)
    {
        bmi088_tim = DWT_GetTimeline_ms();
        BMI088_Read(&BMI088);
        bmi088_tim = DWT_GetTimeline_ms() - bmi088_tim;
        INS.Accel[X] = BMI088.Accel[X];
        INS.Accel[Y] = BMI088.Accel[Y];
        INS.Accel[Z] = BMI088.Accel[Z];
        INS.dgyro[X] = (BMI088.Gyro[X] - INS.Gyro[X])/ (dt) *(1-INS.DGyroLPF) + INS.dgyro[X] * INS.DGyroLPF ;
        INS.dgyro[Y] = (BMI088.Gyro[Y] - INS.Gyro[Y])/ (dt) *(1-INS.DGyroLPF)+ INS.dgyro[Y] * INS.DGyroLPF ;
        INS.dgyro[Z] = (BMI088.Gyro[Z] - INS.Gyro[Z])/ (dt) *(1-INS.DGyroLPF)+ INS.dgyro[Z] * INS.DGyroLPF ;
